...
for a_tag in a_tags[:10]:
    ...
    res = requests.get(hl)
    # 所得した時のtitleが文字化けしてたのを解消
    # https://qiita.com/nittyan/items/d3f49a7699296a58605b
    res.encoding = res.apparent_encoding  # この行を追加
    fav_html = res.text
    fav_soup = BeautifulSoup(fav_html, 'lxml')
    print(fav_soup.title.text)
